...
scaleData EnergyScaleCorrection::GetScaleData(TGPhysObject* temp_electron, int runNumber)
{
    _electron* electron = &temp_electron->particle.electron;
    unsigned r9Bin = 0;
    if (electron->r9 >= 0.94) {
        r9Bin = 1;
    }
...
smearData EnergyScaleCorrection::GetSmearData(TGPhysObject* temp_electron)
{
    _electron* electron = &temp_electron->particle.electron;
    unsigned r9Bin = 0;
    if (electron->r9 >= 0.94) {
...
float EnergyScaleCorrection::GetSmearingFactor(TGPhysObject* temp_electron, int nSigRho, int nSigPhi)
{
    _electron* electron = &temp_electron->particle.electron;

    smearData sdata = GetSmearData(temp_electron);
