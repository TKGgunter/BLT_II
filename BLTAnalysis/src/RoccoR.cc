//Found at https://twiki.cern.ch/twiki/bin/viewauth/CMS/RochcorMuon
////Contact person : Jiyeon Han ( jyhan@fnal.gov ), Arie Bodek ( bodek@pas.rochester.edu ), Aleko Khukhunaishvili ( aleko.khukhunaishvili@gmail.com ) 
////"Rochester correction" is developed to remove a bias of the muon momentum from any detector misalignment or any possible error of the magnetic field. 
...
RoccoR::init(std::string dirname){
    ...
    std::ifstream in(filename.c_str());
		std::cout << "File: " << filename.c_str() << " Initializing Rocco corrections good: " << in.good() << std::endl; 
    ...
    while(std::getline(in, s)){
				...
				for(int m=0; m<sn; ++m){
						...
						//printf("File %s OK? %d %d\n", inputfile.c_str(), gSystem->AccessPathName(inputfile.c_str()), access(inputfile.c_str(), F_OK));
						...
						if(gSystem->AccessPathName(inputfile.c_str())) {
							...
							//printf("before v push back %s\n", inputfile.c_str());
							v.push_back(RocOne(inputfile, 0, si, m));
							//printf("after v push back\n");
						}
				}
				//printf("before RC push back\n");
				RC.push_back(v);
				//printf("after RC push back\n");
    }
