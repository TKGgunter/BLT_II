...
std::string variables =	"lep1_pt float;"
								...
								//** Electron Trigger Variables begin here **//
												"ele_trigger_fire int;"
												;	
...
std::vector<std::string> trigger_vector = { "HLT_Ele27_WPTight_Gsf_v*", 
...
void DemoAnalyzer::Begin(TTree *tree)
{
		...
		eleTrigger_plot		= TH1F("ele", "ele",50, 0.0, 50.); 
		ele_plot		      = TH1F("eleOFF", "eleOFF",50, 0.0, 50.); 
		ratio_ele_plot		= TH1F("ratioele", "ratioele",50, 0.0, 50.); 
...
Bool_t DemoAnalyzer::Process(Long64_t entry)
{
    ...
    *get_value(&vars_float, "gen_weight")     = gen_event_info_weight;
		...
		{// BJet weights
				WeightAndSigma bjet_weight = {1.0, 0.0, 0.0};
				FOR_IN(jet, jetList){
						...
						bjet_weight.up   += pow(temp_weight.up, 2);
						bjet_weight.down += pow(temp_weight.down, 2);
				}
				bjet_weight.up   = pow(bjet_weight.up, 0.5);
				bjet_weight.down = pow(bjet_weight.down, 0.5);
...

		{
				...
				*get_value(&vars_float, "weight")  = pu_weight * lep1_weight * lep2_weight * bjet_weight;
				if(isRealData){ 
    ...
    *get_value(&vars_float, "qt")                     = leptonList.size() >= 2 ? qtCalc(&leptonList[0], &leptonList[1]) : -999.99;
    ...
    if (leptonList.size() >= 2){
				...
				*get_value(&vars_float, "recoil")		= recoil_vec.Pt();
		}	
		...
		if (jetList.size() > 0){
				...
				//Look above.
				*get_value(&vars_float, "recoil_res")							= (recoil_vec - _recoil + _recoil_res).Pt();
				*get_value(&vars_float, "recoil_scale_up")				= (recoil_vec - _recoil + _recoil_scale_up).Pt();
				*get_value(&vars_float, "recoil_scale_down")			= (recoil_vec - _recoil + _recoil_scale_down).Pt();
		}
    ...
    *get_value(&vars_float, "met_jetscale_down")    = fInfo->pfMET - tot_jetscale_down;
		...
		//Z mass cut
		{
				...
				float mll = *get_value(&vars_float, "mll");
				if (same_flavor) {
						if ( fabs(mll - 91.0) > 15 ) END_RECORDING(nEvents, true)
				}
			  if ( *get_value(&vars_int, "lep1_type")  != 11 ) END_RECORDING(nEvents, true)
		}
...
*/
		...
		//NOTE
		//To test the effects of loose electron trigger when applying 28 lead lepton cut pt cut
		if ( *get_value(&vars_float, "lep1_pt")  <= 28 ) END_RECORDING(nEvents, true)
		...
		//TODO
		//THINK ABOUT THIS
		//Determining electron eff
		if (fInfo->triggerBits != 0){
				*get_value(&vars_int, "ele_trigger_fire")  = -1;
				for(unsigned i = 0; i < trigger_vector.size(); i++){
						ele_plot.Fill( *get_value(&vars_float, "lep1_pt") );
						if ( triggerSelector->pass( trigger_vector[i], fInfo->triggerBits) ){
								eleTrigger_plot.Fill( *get_value(&vars_float, "lep1_pt") );
								ratio_ele_plot.Fill( *get_value(&vars_float, "lep1_pt") );
								*get_value(&vars_int, "ele_trigger_fire")  += 1+i;
						}
				}
		}
...
void DemoAnalyzer::Terminate()
{
    ...
    //
		ratio_ele_plot.Divide(&ele_plot);
...
//		std::cout << " GEN       : Muons "  << gMuons << "\t" << "Electrons " << gElectrons << std::endl;
    std::cout << "  ============================================================" << std::endl;
		...
		//Writing both out might be causing trouble
