...
std::string variables =	"lep1_pt float;"
                        ...
                        "dressed_lep1_pt float;"
                        "dressed_lep2_pt float;"
                        "dressed_lep1_eta float;"
                        "dressed_lep2_eta float;"
                        "dressed_lep1_phi float;"
                        "dressed_lep2_phi float;"
...
Bool_t DemoAnalyzer::Process(Long64_t entry)
{
    ...
    std::vector<TGPhysObject> bare_leptons;
    std::vector<TGPhysObject> dressed_leptons;
    ...
    float gen_event_info_weight = 1.0;
		{
            ...
            FOR_IN_fARR(lepton, fGenParticleArr, TGenParticle){
                if( abs(lepton->pdgId) == 11 || abs(lepton->pdgId) == 13 || abs(lepton->pdgId) == 15 ){
                    //NOTE
                    //Status code
                    //https://twiki.cern.ch/twiki/bin/view/CMSPublic/WorkBookGenParticleCandidate
                    if( lepton->status == 1 ){ 
                        bare_leptons.push_back(TGPhysObject(lepton));
                    }
                }
            }
            FOR_IN(lepton, bare_leptons){
                TGPhysObject _lepton = *lepton; 
                FOR_IN_fARR(photon, fGenParticleArr, TGenParticle){
                    if( abs(photon->pdgId) == 22  && photon->status == 1 ) {
                        if( dR( photon->phi, lepton->phi(), photon->eta, lepton->eta() )  < 0.1 ) {
                            auto v4 = lepton->p4() + TGPhysObject(photon).p4();
                            _lepton.set_p4(v4);
                        }
                    }
                }

                dressed_leptons.push_back(_lepton);
            }
            tgSort(bare_leptons);
            tgSort(dressed_leptons);
    ...
    *get_value(&vars_float, "pu_weight_down") = weights.GetPUWeight(fInfo->nPUmean).down;
    ...
    if ( bare_leptons.size() >= 2){
        *get_value(&vars_float, "bare_lep1_pt") = bare_leptons[0].pt();
        *get_value(&vars_float, "bare_lep2_pt") = bare_leptons[1].pt();

        *get_value(&vars_float, "bare_lep1_eta") = bare_leptons[0].eta();
        *get_value(&vars_float, "bare_lep2_eta") = bare_leptons[1].eta();

        *get_value(&vars_float, "bare_lep1_phi") = bare_leptons[0].phi();
        *get_value(&vars_float, "bare_lep2_phi") = bare_leptons[1].phi();
        *get_value(&vars_float, "bare_mll") =  bare_leptons.size() >= 2 ? ((bare_leptons[0].p4()) + (bare_leptons[1].p4())).Mag() : -999.99;
    }
    //NOTE
    //There seems to be a problem with no bare leptons in an event. I think this is because there are no
    //status 1 leptons. I'm not sure what is causing this. The effect is about 3%.
    //else{  printf("Events with no 2 bare leptons %d\n", int(bare_leptons.size()));  }

    //BARELY DRESSED LEPTONS
    if ( dressed_leptons.size() >= 2){
        *get_value(&vars_int, "lep1_type")  = abs(dressed_leptons[0].particle.genparticle.pdgId);
        *get_value(&vars_int, "lep2_type")  = abs(dressed_leptons[1].particle.genparticle.pdgId);

        *get_value(&vars_int, "lep1_q")  = dressed_leptons[0].q();
        *get_value(&vars_int, "lep2_q")  = dressed_leptons[1].q();


        *get_value(&vars_float, "dressed_lep1_pt")  = dressed_leptons[0].pt();
        *get_value(&vars_float, "dressed_lep2_pt")  = dressed_leptons[1].pt();

        *get_value(&vars_float, "dressed_lep1_eta") = dressed_leptons[0].eta();
        *get_value(&vars_float, "dressed_lep2_eta") = dressed_leptons[1].eta();

        *get_value(&vars_float, "dressed_lep1_phi") = dressed_leptons[0].phi();
        *get_value(&vars_float, "dressed_lep2_phi") = dressed_leptons[1].phi();
        *get_value(&vars_float, "dressed_mll") =  dressed_leptons.size() >= 2 ? ((dressed_leptons[0].p4()) + (dressed_leptons[1].p4())).Mag() : -999.99;
    }
